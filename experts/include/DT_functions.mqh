...
void createGlobal( string name, double value ){
  static string sym = "";
  if( sym == "" ){
    sym = StringConcatenate( getSymbol(), "_" );
  }
  name = StringConcatenate( sym, name );
...
void setGlobal( string name, double value ){
  ...
  GlobalVariableSet( StringConcatenate( sym, name ), value );
}
...
double getGlobal( string name ){
  ...
  return ( GlobalVariableGet(StringConcatenate( sym, name )) );
}
