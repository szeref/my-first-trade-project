...
// ###########################################################  Find NEW LMIT Positons  ############################################################         
  int i, o_type, len = ArrayRange( TLINES, 0 );
  ...
  for( i = 0; i < len; i++ ){
    ...
    if( !setPositionPrices( o_type, tp, sl, tLine_price, TLINES[i][TL_PERI] ) ){
      errorCheck( StringConcatenate( Symbol()," Set new Limit position failed: ",  TLINES[i][TL_NAME] ) );
      log( StringConcatenate( Symbol()," Set new Limit position failed: ", TLINES[i][TL_NAME] ), 17.0, StrToDouble(TLINES[i][TL_ID]) );
      continue;
    }
    
    // comment = StringConcatenate( TLINES[i][TL_TYPE], " ", TLINES[i][TL_PERI], " ", DoubleToStr( fibo_100, 5 ) );
    
    OrderSend( Symbol(), o_type, TRADE_LOT, op, 5, sl, tp, TLINES[i][TL_NAME], 3, TimeCurrent()+5400, Orange );
...
bool setPositionPrices( int o_type, double& tp, double& sl, double op, string line_period ){
	if( line_period == "H4" ){
		if( o_type % 2 == 0 ){
