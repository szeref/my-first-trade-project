...
extern color PivotColor= Magenta;
extern color SuportColor= Blue;
extern color ResistanceColor= Red;

extern bool ShowMidPivot = true;
extern color MidPivotColor= LightGray;
extern int LineStyleMidP= 2;
...
int start()
{
   ...
   SetTimeLine("YesterdayStart", "yesterday", idxfirstbarofyesterday, DayLabel, level - 4*Point);
   SetTimeLine("YesterdayEnd", "today", idxfirstbaroftoday, DayLabel, level - 4*Point);
   ...
   if (ShowSweetSpots) {
      ...
      SetLevel(DoubleToStr(ds1,Digits), ds1,  SweetSpotsColor, LineStyle, LineThickness, Time[10]);
      SetLevel(DoubleToStr(ds2,Digits), ds2,  SweetSpotsColor, LineStyle, LineThickness, Time[10]);
   ...
   if (ShowPivots==true) {
      ...
      SetLevel("S1", s1,      SuportColor, LineStyle, LineThickness, startofday);
      SetLevel("S2", s2,      SuportColor, LineStyle, LineThickness, startofday);
      SetLevel("S3", s3,      SuportColor, LineStyle, LineThickness, startofday);
   }
...
void SetLevel(string text, double level, color col1, int linestyle, int thickness, datetime startofday)
{
   ...
   ObjectSetText(labelname, pricelabel, 8, "Arial", LabelColor);
}
...
void SetTimeLine(string objname, string text, int idx, color col1, double vleveltext) 
{
   ...
   ObjectSet(name, OBJPROP_COLOR, DaySeparator);
